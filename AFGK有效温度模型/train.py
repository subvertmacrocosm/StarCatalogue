...
for i in range(epoch):
    ...
    if total_loss / (valid_dataset_size//batch +1) < best_loss:
        best_loss = total_loss / (valid_dataset_size//batch +1)
        best_epoch = i
        torch.save(net.state_dict(), "nb_star_dic_{}_{}.pth".format(i,best_loss))
    print("{}th end\nuse_time: {}s  total: {}s".format(test_step, end_time - start_time, end_time - total_start_time))
    print("valid loss: {}\nbest: {}th vl: {}".format(total_loss / (valid_dataset_size//batch +1), best_epoch, best_loss))
